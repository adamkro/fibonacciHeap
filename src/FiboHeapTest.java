...
public class FiboHeapTest {
    ...
    public static int getNumMarked(FibonacciHeap heap) {
        return (heap.potential() - heap.getPotentialTrees()) / 2;
    }
...
//                FiboHeapPrinter.printHeap(heap);
    ...
    public static void testMeld(FibonacciHeap heap, int upperBound) {
        ...
        int size = heap.size(), numOfTrees = heap.getPotentialTrees(), numMarked = getNumMarked(heap);
        heap.meld(heap2);
        if (heap.size() != size + heap2.size()) {
            throw new RuntimeException("size after meld is not sum of both heaps");
        }
        if (heap.getPotentialTrees() != numOfTrees + heap2.getPotentialTrees()) {
...
//            }
//        } else {
//            FibonacciHeap.HeapNode cur = heap.getRoot();
//            FibonacciHeap.HeapNode next;
//            Random rand = new Random(0L);
//            int kMin;
//            int[] kMinVals, dumbKMinVals;
//            FibonacciHeap heaps[] = new FibonacciHeap[heap.getPotentialTrees()];
//            int heapIndex = 0;
//            do {
//                next = cur.getRight();
//                cur.setLeft(cur);
...
//                heapIndex++;
//                cur = next;
//            } while (next != null && next != heap.getRoot());
...
//    }
    ...
    public static int calculateNumOfTrees(FibonacciHeap heap) {
        ...
        FibonacciHeap.HeapNode cur = heap.getRoot().getRight();
        while (cur != heap.getRoot()) {
            i++;
            cur = cur.getRight();
        }
    ...
    public static void testNumOfTrees(FibonacciHeap heap) {
        ...
        if (actualNumOfTrees != heap.getPotentialTrees()) {
            throw new RuntimeException(String.format("should have %s trees but counted %s", heap.getPotentialTrees(), actualNumOfTrees));
        }
    }
    ...
    public static void testHeapFields(FibonacciHeap heap, ExpectedFields expected) {
        if (expected.numOfTrees != null && expected.numOfTrees != heap.getPotentialTrees()) {
            throw new RuntimeException(
                    String.format("wrong amount of trees. found %d but expected %d",
                            heap.getPotentialTrees(), expected.numOfTrees)
            );
        }
